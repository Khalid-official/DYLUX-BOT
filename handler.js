        ...
        for (let name in global.plugins) {
            ...
            if ((usedPrefix = (match[0] || '')[0])) {
                ...
                if (!isPrems && plugin.diamond && global.db.data.users[m.sender].diamond < plugin.diamond * 1) {
                    this.reply(m.chat, `✳️ Your diamonds are sold out/use the following command to buy more diamonds \n*${usedPrefix}buy* <cantidad> \n*${usedPrefix}buyall*`, m)
                ...
                try {
                    ...
                    if (m.diamond)
                        m.reply(`you have used *${+m.diamond}* 💎`)
                }
                break
            }
        }
...
export async function participantsUpdate({ id, participants, action }) {
    ...
    switch (action) {
        ...
        case 'demote':
            ...
            if (!text)
                text = (chat.sDemote || this.sdemote || conn.sdemote || '@user is not ad admin anymore')
...
export async function groupsUpdate(groupsUpdate) {
    ...
    for (const groupUpdate of groupsUpdate) {
        ...
        if (groupUpdate.desc) text = (chats.sDesc || this.sDesc || conn.sDesc || 'Description changed to \n@desc').replace('@desc', groupUpdate.desc) if (groupUpdate.subject) text = (chats.sSubject || this.sSubject || conn.sSubject || 'Group name changed to \n@group').replace('@group', groupUpdate.subject) if (groupUpdate.icon) text = (chats.sIcon || this.sIcon || conn.sIcon || 'Group icon changed to').replace('@icon', groupUpdate.icon) if (groupUpdate.revoke) text = (chats.sRevoke || this.sRevoke || conn.sRevoke || 'Group link changes to\n@revoke').replace('@revoke', groupUpdate.revoke)
        if (!text) continue
...
global.dfail = (type, m, conn) => {
    let msg = {
        rowner: '👑 This command can only be used by the *Bot Creator*', owner: '🔱 This command can only be used by the *Bot Owner*', mods: '🔰 This feature is only for *For Bot moderators*', premium: '💠 This command is only for *Premium* members\n\nType */premium* for more info', group: '⚙️ This command can only be used in groups!', private: '📮 This command can only be used in the Bot's *private* chat', admin: '🛡️ This command is only for *Admins* of the group', botAdmin: '💥 To use this command I must be *Administrator!*', unreg: '📇 Register to use this function By writing:\n\n*/reg name.age*\n\n📌Example: */reg dylux.16*', restrict: '🔐 This feature is *disabled*'
    }[type]
