        ...
        for (let name in global.plugins) {
            ...
            if ((usedPrefix = (match[0] || '')[0])) {
                ...
                if (!isPrems && plugin.diamond && global.db.data.users[m.sender].diamond < plugin.diamond * 1) {
                    this.reply(m.chat, `✳️ Your diamonds are sold out\nuse the following command to buy more diamonds\n\n*${usedPrefix}buy*`, m)
            ...
            if (chat.welcome) {
                ...
                for (let user of participants) {
                    ...
                    try {
                        ...
                        ppgp = await this.profilePictureUrl(id, '')
                        } finally {
                        text = (action === 'add' ? (chat.sWelcome || this.welcome || conn.welcome || '').replace('', await this.getName(id)).replace('', groupMetadata.desc?.toString() || '') :
                            (chat.sBye || this.bye || conn.bye || '')).replace('', '' + user.split('')[0])
...
export async function groupsUpdate(groupsUpdate) {
    ...
    for (const groupUpdate of groupsUpdate) {
        ...
        if (groupUpdate.subject) text = (chats.sSubject || this.sSubject || conn.sSubject || 'The group name changed a \n@group').replace('@group', groupUpdate.subject)
...
≡ Deleted a message ┌─⊷ 𝘼𝙉𝙏𝙄 𝘿𝙀𝙇𝙀𝙏𝙀 ▢ *Name :* @${participant.split`@`[0]} └───────────── To disable this feature, type */off antidelete* *.enable delete*
`.trim(), msg, {
...
watchFile(file, async () => {
    unwatchFile(file)
    console.log(chalk.magenta("✅  'handler.js' was updated"))
